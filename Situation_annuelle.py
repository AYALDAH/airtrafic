...
# Trier le DataFrame par volume décroissant
    ...
    # Ajouter les entités à la liste des entités ayant le plus de volume parmi les mois ayant le plus de volume
        ...
        st.write("**Détails par Entité**","## text_size")
        with st.sidebar:
...
# Group the data by month and site, and calculate the sum of volume for each month
        monthly_data_groupedc = filtered_data.groupby([pd.Grouper(key='DATE', freq='M')])['VOLUME'].sum().reset_index()
        monthly_data_groupedc['Change'] = monthly_data_groupedc['VOLUME'].diff().fillna(0)

# Create the waterfall chart using Plotly Express
        fig_waterfall = px.bar(monthly_data_groupedc, x='DATE', y='Change', title='Variation du volume moyen en 2022', barmode='overlay', labels={'DATE': 'Date', 'Change': 'Change in Volume'},color='Change',color_continuous_scale='RdBu,color_continuous_midpoint=0)
